...
def L_color(x):
    ...
    # Use tf.pow instead of K.pow
    l = tf.pow(Drg + Drb + Dgb, 0.5)
...
def L_TV(x, TVLoss_weight=1):
    batch_size = tf.cast(tf.shape(x)[0], tf.float32)  # Cast batch_size to float32
    h_x = tf.shape(x)[1]
    w_x = tf.shape(x)[2]
    
    # Cast count_h and count_w to float32 for consistency in operations
    count_h = tf.cast((tf.shape(x)[2] - 1) * tf.shape(x)[3], tf.float32)
    count_w = tf.cast(tf.shape(x)[2] * (tf.shape(x)[3] - 1), tf.float32)
    
    h_tv = tf.reduce_sum(tf.pow((x[:, 1:, :, :] - x[:, :h_x - 1, :, :]), 2))
    w_tv = tf.reduce_sum(tf.pow((x[:, :, 1:, :] - x[:, :, :w_x - 1, :]), 2))
    
    # Return the TV loss with consistent dtype
...
def L_spa(org, enhance):
    ...
    D_enhance_left = Conv2D(kernel_initializer=init_f_l, padding='same', kernel_size=(3, 3), filters=1)(enhance_pool)
    ...
    D_left = tf.pow(D_org_left - D_enhance_left, 2)
